...
class TestGetOpenaiApiBase:
    ...
    def test_reads_openai_api_base(self):
        with patch.dict(
            os.environ, {"OPENAI_API_BASE": "https://relay.example/v1"}, clear=True
        ):
            assert get_openai_api_base() == "https://relay.example/v1"

    def test_reads_openai_base_url(self):
        with patch.dict(
            os.environ, {"OPENAI_BASE_URL": "https://other.example/v1"}, clear=True
        ):
    ...
    def test_openai_api_base_takes_precedence_over_base_url(self):
        with patch.dict(
            os.environ,
            {
                "OPENAI_API_BASE": "https://primary.example/v1",
                "OPENAI_BASE_URL": "https://secondary.example",
            },
            clear=True,
        ):
...
class TestSettingsApiBase:
    ...
    def test_openai_api_base_from_env(self):
        with patch.dict(
            os.environ, {"OPENAI_API_BASE": "https://api.example/v1"}, clear=True
        ):
            s = Settings()
